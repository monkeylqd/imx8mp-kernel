...
struct imx6_pcie {
	...
	int			dis_gpio;
	int			reset_gpio;
	int			rm500q_reset_gpio;
	int			rm500q_power_off;
	int			pcie_clkreq;
...
static void imx6_pcie_deassert_core_reset(struct imx6_pcie *imx6_pcie)
{
	...
	/* After power up more than 33ms, rm500q reset. */
	if (gpio_is_valid(imx6_pcie->rm500q_reset_gpio)) {
		gpio_set_value_cansleep(imx6_pcie->rm500q_reset_gpio, 1);
		msleep(20);
		gpio_set_value_cansleep(imx6_pcie->rm500q_reset_gpio, 0);
	} else {
		dev_err(dev, "rm500q_reset gpio is invalid\n");
	}
	msleep(100);

	/* After RM500Q reset, rm500q power on. */
	if (gpio_is_valid(imx6_pcie->rm500q_power_off)) {
		gpio_set_value_cansleep(imx6_pcie->rm500q_power_off, 0);
		msleep(20);
		gpio_set_value_cansleep(imx6_pcie->rm500q_power_off, 1);
	} else {
		dev_err(dev, "rm500q_power_off gpio is invalid\n");
	}
	msleep(30);

	/* Set pcie_clkreq gpio */
	if (gpio_is_valid(imx6_pcie->pcie_clkreq)) {
		gpio_set_value_cansleep(imx6_pcie->pcie_clkreq,	1);
	} else {
		dev_err(dev, "pcie_clkreq gpio is invalid\n");
	}
	msleep(10000);
	...
	if (gpio_is_valid(imx6_pcie->reset_gpio)) {
		...
		dev_err(dev, "reset_gpio gpio is invalid\n");
	}
...
static int imx6_pcie_probe(struct platform_device *pdev)
{
	...
	imx6_pcie->rm500q_reset_gpio = of_get_named_gpio(node, "rm500q-reset-gpio", 0);
	imx6_pcie->rm500q_power_off = of_get_named_gpio(node, "rm500q-power-off", 0);
	imx6_pcie->pcie_clkreq = of_get_named_gpio(node, "pcie-clkreq", 0);
	...
	/* rm500q-reset-gpio */
	if (gpio_is_valid(imx6_pcie->rm500q_reset_gpio)) {
		ret = devm_gpio_request_one(dev, imx6_pcie->rm500q_reset_gpio,
					GPIOF_OUT_INIT_HIGH,
				"rm500q_reset_gpio reset");
		if (ret) {
			dev_err(dev, "unable to get rm500q_reset_gpio\n");
		}
	} else if (imx6_pcie->rm500q_reset_gpio == -EPROBE_DEFER) {
		return imx6_pcie->rm500q_reset_gpio;
	}

	/* rm500q-power-off */
	if (gpio_is_valid(imx6_pcie->rm500q_power_off)) {
		ret = devm_gpio_request_one(dev, imx6_pcie->rm500q_power_off,
					GPIOF_OUT_INIT_LOW,
				"rm500q_power_off reset");
		if (ret) {
			dev_err(dev, "unable to get rm500q_power_off gpio\n");
		}
	} else if (imx6_pcie->rm500q_power_off == -EPROBE_DEFER) {
		return imx6_pcie->rm500q_power_off;
	}

	/* pcie-clkreq */
	if (gpio_is_valid(imx6_pcie->pcie_clkreq)) {
		ret = devm_gpio_request_one(dev, imx6_pcie->pcie_clkreq,
					GPIOF_OUT_INIT_LOW,
				"pcie_clkreq reset");
		if (ret) {
			dev_err(dev, "unable to get pcie_clkreq gpio\n");
		}
	} else if (imx6_pcie->pcie_clkreq == -EPROBE_DEFER) {
		return imx6_pcie->pcie_clkreq;
	}
